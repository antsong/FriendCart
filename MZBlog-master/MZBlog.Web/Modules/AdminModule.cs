...
namespace MZBlog.Web.Modules
{
    public class AdminModule : SecureModule
    {
            ...
            : base(viewProjectionFactory, commandInvokerFactory)
        {
            ...
            Post["/mz-admin/change-password"] = _ => ChangePassword(this.BindAndValidate<ChangePasswordCommand>());
        ...
        private dynamic ChangePassword(ChangePasswordCommand command)
        {
            if (!ModelValidationResult.IsValid)
            {
                ...
                {
                    foreach (var errMsg in err.Value)
                    {
                        AddMessage(errMsg, "warning");
                    }
                    //AddMessage("请正确输入旧密码和新密码", "danger");
                }
