...
namespace MZBlog.Web.Modules
{
    public class HomeModule : FrontModule
    {
        ...
        public HomeModule(IViewProjectionFactory viewFactory, ISpamShieldService spamShield, ICommandInvokerFactory commandInvokerFactory)
            : base(viewFactory, commandInvokerFactory)
        {
            ...
            Post["/mz-search"] = p =>
                ReturnSearch(new SearchBlogPostsBindingModel() { SearchContent = Request.Form["content"] });
        ...
        public dynamic ReturnHomeAction(RecentBlogPostsBindingModel input)
        {
            ...
            if (model.Posts.Count() == 0)
            {
                if (input.Page > 1)
                    ...
                    return Response.AsText("AntBlog尚未发现任何已经发布的文章哦!", "text/html; charset=utf-8");
            }
            if (input.Page == 1)
                ViewBag.Title = "首页";
            else
        ...
        public dynamic ReturnSearch(SearchBlogPostsBindingModel input)
        {

            var model = _viewFactory.Get<SearchBlogPostsBindingModel, SearchBlogPostsViewModel>(input);
            if (model == null)
                return HttpStatusCode.NotFound;

            ViewBag.Title = "搜索:" + model.SearchContent;
