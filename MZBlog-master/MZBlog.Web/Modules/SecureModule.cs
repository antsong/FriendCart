...
namespace MZBlog.Web.Modules
{
    public class SecureModule : BaseNancyModule
    {
        ...
        public SecureModule(IViewProjectionFactory viewProjectionFactory, ICommandInvokerFactory commandInvokerFactory)
            :base(commandInvokerFactory)
        {
        ...
        private Response SetCurrentUserToViewBag(NancyContext ctx)
        {
            ...
            if (author == null)
                return ctx.GetRedirect("~/mz-login?returnUrl=" + Request.Url.BasePath + Request.Url.Path).WithCookie(FormsAuthentication.CreateLogoutCookie());
