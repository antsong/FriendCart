...
namespace MZBlog.Web.Security
{
    public class FormsAuthentication
    {
        ...
        {
            var dtcodtdCookie = cookieValue;//HttpUtility.UrlDecode(cookieValue);
        ...
        {
            return CreateAuthCookie(username, false);
        }

        public static NancyCookie CreateAuthCookie(string username, bool createPersistentCookie)
        {
            var encryptedCookieValue = EncryptAndSignCookie(username);

            return createPersistentCookie
                ? new NancyCookie(FormsAuthenticationCookie, encryptedCookieValue, true, false,
        ...
        {
            return new NancyCookie(FormsAuthenticationCookie, "", true, false) { Expires = DateTime.UtcNow.AddDays(-30) };
        }
